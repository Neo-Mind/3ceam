...
void initChangeTables(void)
{
	...
	set_sc( WL_MARSHOFABYSS      , SC_MARSHOFABYSS    , SI_MARSHOFABYSS    , SCB_AGI|SCB_DEX|SCB_ASPD|SCB_SPEED );
	set_sc( WL_RECOGNIZEDSPELL   , SC_RECOGNIZEDSPELL , SI_RECOGNIZEDSPELL , SCB_MAXSP|SCB_ALL|SCB_MATK );
	add_sc( WL_STASIS            , SC_STASIS );
	set_sc( WL_HELLINFERNO       , SC_BURNING         , SI_BLANK           , SCB_REGEN );
	...
	set_sc( SC_SHADOWFORM        , SC_SHADOWFORM_        , SI_SHADOWFORM        , SCB_NONE );
	set_sc( SC_BODYPAINT         , SC_BODYPAINT_         , SI_BODYPAINTING      , SCB_ASPD );
	set_sc( SC_INVISIBILITY      , SC_INVISIBILITY_      , SI_INVISIBILITY      , SCB_ATK_ELE|SCB_CRI|SCB_ASPD );
	set_sc( SC_ENERVATION        , SC_ENERVATION_        , SI_MASQUERADE        , SCB_BATK );
	set_sc( SC_GROOMY            , SC_GROOMY_            , SI_MASQUERADE        , SCB_ASPD|SCB_HIT|SCB_SPEED );
	set_sc( SC_IGNORANCE         , SC_IGNORANCE_         , SI_MASQUERADE        , SCB_NONE );
	set_sc( SC_LAZINESS          , SC_LAZINESS_          , SI_MASQUERADE        , SCB_FLEE );
	set_sc( SC_UNLUCKY           , SC_UNLUCKY_           , SI_MASQUERADE        , SCB_FLEE2 );	// This is used with Autumn Leaves that get Perfect Dodge +5 [pakpil] NOTE: Delete it when confirm this.
	set_sc( SC_WEAKNESS          , SC_WEAKNESS_          , SI_MASQUERADE        , SCB_MAXHP );
	set_sc( SC_STRIPACCESSARY    , SC_STRIPACCESSORY     , SI_STRIPACCESSORY    , SCB_DEX|SCB_INT|SCB_LUK );
	set_sc( SC_MANHOLE           , SC_MANHOLE_           , SI_MANHOLE           , SCB_NONE );
	add_sc( SC_CHAOSPANIC        , SC_CONFUSION  );
	set_sc( SC_BLOODYLUST        , SC_PROVOKE            , SI_BLANK             , SCB_BATK|SCB_DEF|SCB_DEF2 );
	...
	// Hallucination Walk penalty SC
	StatusChangeFlagTable[SC_HALLUCINATIONWALK_DELAY] |= SCB_ASPD | SCB_SPEED;
	// New Poisons
	StatusChangeFlagTable[SC_PYREXIA] = SCB_NONE;
	StatusChangeFlagTable[SC_PARALIZE] |= SCB_ASPD|SCB_FLEE|SCB_SPEED;
	StatusChangeFlagTable[SC_DISHEART] = SCB_NONE;
	StatusChangeFlagTable[SC_MUSHROOM] = SCB_REGEN;
	StatusChangeFlagTable[SC_VENOMBLEED] = SCB_MAXHP;
	StatusChangeFlagTable[SC_TOXIN] = SCB_REGEN;
	StatusChangeFlagTable[SC_OBLIVIONCURSE] = SCB_REGEN;
	StatusChangeFlagTable[SC_LEECHEND] = SCB_NONE;
...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	if( hp && !(flag&1) ) {
		if( sc ) {
			...
			status_change_end(target,SC_VOICEOFSIREN,-1);
			status_change_end(target,SC_DEEPSLEEP,-1);
			...
			if ( (sce = sc->data[SC_CLOAKINGEXCEED]) )
			{
				sce->val3 --;
				if( sce->val3 <= 0 )
...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int flag)
{
	...
	if(sc && sc->count)
	{
		...
		if (skill_num && //Do not block item-casted skills.
			...
			if( sc->data[SC_MANHOLE_] || ((tsc=status_get_sc(target)) && tsc->data[SC_MANHOLE_]) )
				return 0;
	...
	case BL_PC:
		{
			...
			if ( sd->sc.data[SC_MANHOLE_] )
				return 0;
			if ( sc && sc->data[SC_CURSEDCIRCLE_] )
				return 0;
		}
		break;
	...
	{	//Max HP setting from Summon Flora/marine Sphere
		...
		{	// different levels of HP according to skill level
			...
			if (ud->skillid == NC_SILVERSNIPER) {
				status->mode|= MD_CANATTACK|MD_AGGRESSIVE;
				status->batk += 100 * ud->skilllv - 100;
			}
			else
			if (ud->skillid == NC_MAGICDECOY) {
				status->mode|= MD_CANATTACK|MD_AGGRESSIVE;
				status->matk_min += 250 + ud->skilllv * 50;
				status->matk_max += 250 + ud->skilllv * 50;
			}
...
// ----- STATS CALCULATION -----
	...
	if((skill=pc_checkskill(sd,AC_OWL))>0)
		status->dex += skill;
	if((skill=pc_checkskill(sd,RA_RESEARCHTRAP))>0) //Added RA_RESEARCHTRAP Bonus for INT [Jobbie].
		status->int_ += skill;
...
// ----- SP MAX CALCULATION -----
	...
	if((skill=pc_checkskill(sd,RA_RESEARCHTRAP))>0)//Added RA_RESEARCHTRAP Bonus for SP [Jobbie].
...
void status_calc_regen(struct block_list *bl, struct status_data *status, struct regen_data *regen)
{
	...
	if( sd )
	{
		...
		if( (skill=pc_checkskill(sd,NJ_NINPOU)) > 0 )
			val += skill*3 + skill*status->max_sp/500;
		if( (skill=pc_checkskill(sd,WM_LESSON)) > 0 )
			val += skill*3 + skill*status->max_sp/1000;
...
// if(flag&SCB_RANGE)
	...
	if(flag&SCB_MATK) {
		...
		//FIXME: Is this the right place? This code was relocated due to eAthena's new code structure. [Jobbie]
		// NOTE: if Recognized spell works on BL_PC only uncoment the first line and dell the second one.
		// else, delete the firs one or make the necesary modifications. [pakpil]
		//if(sd && sd->sc.data[SC_RECOGNIZEDSPELL]) { // Maximum damage for Recognized Spell [LimitLine]
		if(sc->data[SC_RECOGNIZEDSPELL]) { // Maximum damage for Recognized Spell [LimitLine]
...
static unsigned short status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	...
	if(sc->data[SC_CONCENTRATE] && !sc->data[SC_QUAGMIRE] && !sc->data[SC_MARSHOFABYSS])
...
static unsigned short status_calc_dex(struct block_list *bl, struct status_change *sc, int dex)
{
	...
	if(sc->data[SC_MARSHOFABYSS])
		dex -= sc->data[SC_MARSHOFABYSS]->val2;
	if(sc->data[SC_BLESSING]){
		if (sc->data[SC_BLESSING]->val2)
...
//		batk -= batk * 25/100;
	...
	if(sc->data[SC_WARCRYOFBEYOND])
		batk += batk / 100 * 15 * sc->data[SC_WARCRYOFBEYOND]->val1;	// Needs official value. [LimitLine]
	if(sc->data[SC_ENERVATION_])
		batk -= batk * sc->data[SC_ENERVATION_]->val2 / 100;
	if(sc->data[SC_GN_CARTBOOST])
		batk += sc->data[SC_GN_CARTBOOST]->val3;
...
static unsigned short status_calc_matk(struct block_list *bl, struct status_change *sc, int matk)
{
	...
	if(sc->data[SC_MOONLIT])
		matk += matk / 100 * 10 * sc->data[SC_MOONLIT]->val1;
	if(sc->data[SC_MELODYOFSINK])
		matk += matk / 100 * 15 * sc->data[SC_MELODYOFSINK]->val1;		// Needs official value. [LimitLine]
	if(sc->data[SC_WARCRYOFBEYOND])
		matk -= matk / 100 * 15 * sc->data[SC_WARCRYOFBEYOND]->val1;	// Needs official value. [LimitLine]
...
static signed short status_calc_critical(struct block_list *bl, struct status_change *sc, int critical)
{
	...
	if(sc->data[SC_INVISIBILITY_])
		critical += 20 * sc->data[SC_INVISIBILITY_]->val2;
	if(sc->data[SC_UNLUCKY_])
		critical -= critical * sc->data[SC_UNLUCKY_]->val1 * 10 / 100;
...
static signed short status_calc_hit(struct block_list *bl, struct status_change *sc, int hit)
{
	...
	if(sc->data[SC_GROOMY_])
		hit -= hit * sc->data[SC_GROOMY_]->val3 / 100;
	if(sc->data[SC_FEAR])
		hit -= hit * 20 / 100;
...
static signed short status_calc_flee2(struct block_list *bl, struct status_change *sc, int flee2)
{
	...
	if(sc->data[SC_UNLUCKY_])
		flee2 -= flee2 * sc->data[SC_UNLUCKY_]->val1 * 10 / 100;
...
static signed char status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	...
	if(sc->data[SC_EARTHDRIVE])
		def -= def * sc->data[SC_EARTHDRIVE]->val1 / 100;
...
static signed char status_calc_mdef(struct block_list *bl, struct status_change *sc, int mdef)
{
	...
	if(sc->data[SC_ANALYZE])
		mdef -= 14 * sc->data[SC_ANALYZE]->val1;
	if(sc->data[SC_SYMPHONY])
		mdef += mdef / 5 * sc->data[SC_SYMPHONY]->val1;
...
static signed short status_calc_mdef2(struct block_list *bl, struct status_change *sc, int mdef2)
{
	...
	if(sc->data[SC_ANALYZE])
		mdef2 -= 14 * sc->data[SC_ANALYZE]->val1;
	if(sc->data[SC_SYMPHONY])
		mdef2 += mdef2 / 100 * 20 * sc->data[SC_SYMPHONY]->val1;
	...
	else
	{
		...
		//GetMoveSlowValue()
		{
			...
			else
			{
				...
				if( sc->data[SC_HALLUCINATIONWALK_DELAY] )
					val = max( val, 50 );
				if( sc->data[SC_GROOMY_] )
					val = max( val, sc->data[SC_GROOMY_]->val2 );
		...
		//GetMoveHasteValue1()
		{
			...
			if( sc->data[SC_SIDESLIDE] )
				val = max( val, 100 ); // Need official value. [LimitLine]
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if(!sc->data[SC_QUAGMIRE] || !sc->data[SC_MARSHOFABYSS])
	{
		int max = 0;
	...
	if(sc->data[SC_PARALIZE])
		aspd_rate += 100;
	if(sc->data[SC_GROOMY_])
		aspd_rate += aspd_rate * sc->data[SC_GROOMY_]->val3 / 100;
	if(sc->data[SC_BODYPAINT_])
		aspd_rate -= 5 * sc->data[SC_BODYPAINT_]->val1;
	if(sc->data[SC_INVISIBILITY_])
	...
	if(sc->data[SC_EARTHDRIVE])
		aspd_rate += aspd_rate * sc->data[SC_EARTHDRIVE]->val1 / 100;
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4, int tick,int flag)
{
	...
	struct status_data *status;
	struct view_data *vd;
	int opt_flag, calc_flag, undead_flag, val_flag;
	int duration = tick;
	...
	case SC_ASSNCROS:
	case SC_SWINGDANCE:
	case SC_GN_CARTBOOST: // Fixme: Check if is necesary. [pakpil]
		if (sc->data[SC_QUAGMIRE] || sc->data[SC_MARSHOFABYSS])
			return 0;
	break;
	case SC_CLOAKING:
	...
	case SC_STRIPACCESSORY:
		if (sd) {
			int i;
			opt_flag = 0;
			if(sd->unstripable_equip&EQP_ACC)
				return 0;
	...
	case SC_ASSUMPTIO:
		...
		status_change_end(bl,SC_PRAEFATIO,-1);
		break;
	case SC_KAITE:
		status_change_end(bl,SC_ASSUMPTIO,-1);
		break;
	...
	switch(type)
	{
		...
		case SC_DANCING:
			...
			//val4 : Partner
			if (val1 == CG_MOONLIT)
				clif_status_change(bl,SI_MOONLIT,0,0,0,1,tick);
			val1|= (val3<<16);
		...
		case SC_CLOAKINGEXCEED:
			...
			if (!sd) //Monsters should be able to walk with no penalties and should not have any SP consumption. [Skotlex]
			{ val1 = 5; val2 = 0; }
			else val2 = 10 - pc_checkskill(sd, GC_CLOAKINGEXCEED); //SP cost p/second.
			if( sd )
			switch( pc_checkskill(sd, GC_CLOAKINGEXCEED) ) {	// Max num. of hits before ending Cloaking Exceed. [LimitLine]
				case 1: case 2: val3 = 1;
				case 3: case 4: val3 = 2;
				case 5: val3 = 3;
			}
			if (bl->type == BL_PC) val4 |= battle_config.pc_cloak_check_type&7; //Standard cloaking.
			else val4 |= battle_config.monster_cloak_check_type&7;
			break;
		...
		case SC_THURISAZ:
			...
			break;
		case SC_HAGALAZ:
			val3 = 10;	// Weapon-breaking + Damage reduction success rate.
			break;
		case SC_NAUTHIZ:
			status_change_clear(bl, 2);
			break;
		case SC_WHITEIMPRISON:
			if( bl->id == val2 )	// 20% shorter duration when casted on self. [LimitLine]
				tick  -= tick / 5;
			break;
		case SC_FREEZING:
		...
		case SC_SPELLBOOK_2:
			...
			break;
		case SC_MARSHOFABYSS:
			val2 = 6 * val1; // Agi/Dex decrease.
			if( bl->type == BL_PC )
				val2 /= 2; // Half reduction to players.
			break;
		case SC_ORATIO:
			val2 = val1 * 2;
			break;
		case SC_EXPIATIO:
			if( sd )
			{
				sd->ignore_def[RC_NONBOSS] += 5 * val1;
				sd->ignore_def[RC_BOSS] += 5 * val1;
			}
			break;
		case SC_RENOVATIO:
		...
		case SC_ROLLINGCUTTER:
			if( val1 > 10 )	// Rotation Counter.
				val1 = 10;
			val_flag |= 1; // To show the effect correctly
			break;
		case SC_PYREXIA: // TODO: Add damage, we need know if this damage is caused when sc start or when sc is activated and this cause
						 // damage ramdonly everytime that it's active.
		case SC_PARALIZE:
		case SC_DISHEART:
			clif_specialeffect(bl, 125, AREA);
			break;
		case SC_MUSHROOM:
			val3 = 4;			// Interval.
			if( val3 < 1 ) val3 = 1;
			if( (val4 = tick/(val3 * 1000)) < 1 )
				val4 = 1;
			tick = val3 * 1000;
			clif_specialeffect(bl, 125, AREA);
			break;
		...
		case SC_REPRODUCE_:
			if( sd )
				sd->reproduceskill_id = 0;	// Erase memorized skill.
			break;
		case SC_SHADOWFORM_:
			val2 = val1 + 4; // Number of hits (SkillLV + 4).
			tick = 1000;
			break;
		case SC_INVISIBILITY_:
			val3 = 10 - val2;	// SP% cost.
			if( val3 < 1 )
				val3 = 1;
			tick = 1000;
			break;
		case SC_STRIPACCESSORY:
			if (!sd) //Int,Luk,Dex reduction
				val2 = 20;
			break;
		case SC_SWINGDANCE:
			val2 = val1 * 4;
		...
		case SC_MELODYOFSINK:
			if( sc && sc->data[SC_WARCRYOFBEYOND] )
				status_change_end(bl, SC_WARCRYOFBEYOND, -1);
			break;
		case SC_WARCRYOFBEYOND:
			if( sc && sc->data[SC_MELODYOFSINK] )
				status_change_end(bl, SC_MELODYOFSINK, -1);
			break;
		case SC_ENERVATION_:
			//TODO: find the official chance value and apply it. [pakpil]
			val2 = (val1 + 2) * 10;
			val4 = tick / 1000;
			if(sd)
			...
			break;
		case SC_GROOMY_:
			val2 = (val1 + 2) * 10;	// ASPD reduction
			val3 = (val1 + 1) * 10; // HIT reduction - Assume this value as movement speed reduction too
			val4 = tick / 1000;
			if( sd )
			{
		...
		case SC_WEAKNESS_:
		{
			...
			val4 = tick / 1000;
			if( sd )
			{ // TODO: needed get official rate [pakpil]
				skill_strip_equip(bl, EQP_WEAPON, 100, val1, tick);
				skill_strip_equip(bl, EQP_SHIELD, 100, val1, tick);					
			}
		}
		break;
		case SC_UNLUCKY_:
		...
		case SC_REFLECTDAMAGE:
			...
			break;
		case SC_PRESTIGE:
			val2 = 2 * val1; // Fixme: need official values [pakpil]
			val3 = 2 * val1;
			val4 = tick / 1000;
			break;
		case SC_PIETY:
	...
	switch(type)
	{
		...
		case SC_PYREXIA:	  sc->opt2 |= OPT2_BLIND;		 break;
		//OPT3
		case SC_TWOHANDQUICKEN:
		case SC_ONEHAND:
		case SC_SPEARQUICKEN:
...
int status_change_end(struct block_list* bl, enum sc_type type, int tid)
{
	...
	switch(type){
		...
		case SC_EXPIATIO:
			if( sd )
			{
				sd->ignore_def[RC_NONBOSS] -= 5 * sce->val1;
				sd->ignore_def[RC_BOSS] -= 5 * sce->val1;
			}
			break;
		case SC_WHITEIMPRISON:
			...
			break;
		case SC_HALLUCINATIONWALK:
			if( !status_isdead(bl) )	// Always nice to check for these... [LimitLine]
				sc_start(bl, SC_HALLUCINATIONWALK_DELAY, 100,
					pc_checkskill(sd, GC_HALLUCINATIONWALK), skill_get_time2(GC_HALLUCINATIONWALK, pc_checkskill(sd, GC_HALLUCINATIONWALK)));
			break;
		case SC_HALLUCINATIONWALK_DELAY:
			if( sd )
				status_calc_pc(sd, 0);
			break;
		case SC_JAILED:
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_BURNING:
		if((sce->val4-=3) > 0 )
		{
			int flag, dmg;
			map_freeblock_lock();
			dmg = status->max_hp / 100 * 3 + battle_attr_fix(NULL, bl, 1000, ELE_FIRE, status->def_ele, status->ele_lv);
			clif_damage(bl, bl, gettick(), 0, 0, dmg, 0, 0, 0);
			status_fix_damage(NULL, bl, dmg, 0);
			flag = !sc->data[type];
			map_freeblock_unlock();
			if(flag) return 0;
	...
	case SC_SPELLBOOK_2:
		if( --(sce->val4)%10 == 0 )
		{
			if( !status_zap(bl, 0, sce->val2) )	// No SP to continue. Is this how it should be? [LimitLine]
	...
	case SC_PYREXIA:
		...
		{
			map_freeblock_lock();
			if( rand()%100 <= 50 )	// Need official rate. [LimitLine]
				status_damage(bl, bl, 100, 0, clif_damage(bl, bl, gettick(), 500, 500, 100, -1, 0, 0), 0);
			map_freeblock_unlock();
			sc_timer_next(1000 + gettick(), status_change_timer, bl->id, data);
			return 0;
		}
		break;
	...
	case SC_MUSHROOM:
		if ( --(sce->val4) >= 0 )
		{
			...
			map_freeblock_lock();
			clif_emotion(bl, 18);
			skill_castend_nodamage_id(bl, bl, SA_ABRACADABRA, 5, 0, 0);
			status_damage(NULL, bl, status->hp / 100 * 3, 0, 0, 0);
			map_freeblock_unlock();
			sc_timer_next(4000 + gettick(), status_change_timer, bl->id, data);
			return 0;
		}
		break;
	...
	case SC_LEECHEND:
		if( --(sce->val4) >= 0 )
		{
			...
			if( !sd )
				break;
			map_freeblock_lock();
			status_damage(bl, bl, status->max_hp / 100, 0,
				clif_damage(bl, bl, gettick(), 0, 0, status->max_hp / 100, -1, 0, 0), 0);
			map_freeblock_unlock();
			sc_timer_next(3000 + gettick(), status_change_timer, bl->id, data);
			return 0;
		}
		break;
	...
	case SC_SPHERE_5:
		if(--(sce->val4) > 0)
		{
			...
			{
				sc_timer_next(1000 + gettick(), status_change_timer, bl->id, data); // This value has been optained though packets in RE. Plz, confirm it.
				return 0;
			}
		}
		break;
	...
	case SC_SHADOWFORM_:
		{
			struct block_list *pbl = map_id2bl(sce->val3);
			if( pbl && check_distance_bl(bl, pbl, 7) )
			{
				sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
				return 0;
	...
	case SC_DEEPSLEEP:
		if( --(sce->val4) >= 0 )
		{
			status_heal(bl, status->max_hp / 100, status->max_sp / 100, 0);
			sc_timer_next(sce->val2 * 1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
		break;
	...
	case SC_REFLECTDAMAGE:
		if(--(sce->val4) > 0)
		{
			if( !status_charge(bl, 0, 2) ) // Need official value. [pakpil]
				break;
			sc_timer_next(1000+tick,status_change_timer,bl->id, data);
			return 0;
		}
