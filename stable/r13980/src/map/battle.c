...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	struct status_data *status = status_get_status_data(src); //[Rikter]
	...
	if( sc && sc->data[SC_WHITEIMPRISON] &&
		skill_num != PA_PRESSURE && skill_num != HW_GRAVITATION &&
		skill_num != NJ_ZENYNAGE && skill_num != WL_SOULEXPANSION )
	{	// White Imprison doesn't protect enemy from skills with fixed damage. [LimitLine]
		damage = 0;
		return 0;
	}
	...
	if( sc && sc->count )
	{
		...
		if( tsc && tsc->data[SC_GLOOMYDAY_BUFF] && (skill_num == KN_BRANDISHSPEAR || skill_num == LK_SPIRALPIERCE ||
			skill_num == CR_SHIELDCHARGE || skill_num == CR_SHIELDBOOMERANG || skill_num == PA_SHIELDCHAIN ||
			skill_num == LG_SHIELDPRESS) )
			damage += damage / 10;	// Need official value.
		...
		if( (sce = sc->data[SC_WEAPONBLOCKING]) && flag&BF_WEAPON && sd )
		{
			...
			{
				damage = 0;
				clif_skill_nodamage(&sd->bl, &sd->bl, GC_WEAPONBLOCKING, pc_checkskill(sd, GC_WEAPONBLOCKING),
					sc_start(bl, SC_WEAPONBLOCKING_, 100, sce->val1, skill_get_time2(GC_WEAPONBLOCKING, pc_checkskill(sd, GC_WEAPONBLOCKING))));
			}
		}

		if( (sce = sc->data[SC_HALLUCINATIONWALK]) && flag&BF_MAGIC && (damage > 0) )
		{
		...
		if( (sce = sc->data[SC_CRESCENTELBOW]) && !(flag&BF_LONG) && !(flag&BF_MAGIC) && 
			(src->type == BL_MOB && !(status->mode&MD_BOSS)) && (damage > 0) )
		{
			if( rand()%100 <= 21 + sce->val1 * 3 )
			{
		...
		if( (sce = sc->data[SC_ENERGYGAIN]) && sd && (damage > 0) )
		{
			if( rand()%100 <= 10 + sce->val1 * 5 )
				pc_addspiritball(sd, skill_get_time(MO_CALLSPIRITS, sce->val1), sce->val1);
		}

		if( (sce = sc->data[SC_LIGHTNINGWALK]) && flag&BF_LONG && (damage > 0) )	// Check for targeted magic attacks goes in skill_castend_damage_id. [LimitLine]
		{
			if( rand()%100 < (25 + sce->val1 * 5) )
			{
				short x, y;
				int tx, ty;
				damage = 0;
				...
				clif_slide(bl, x, y);
				clif_skill_poseffect(bl, skill_num, sce->val1, x, y, gettick());
		...
		if(((sce = sc->data[SC_KYRIE]) || (sce = sc->data[SC_PRAEFATIO])) && damage > 0){
			sce->val2-=damage;
	...
	if(skill_num){
		...
		switch(skill_num)
		{
			...
			case SR_FALLENEMPIRE:
				if( tsd )
				{
					wd.div_ += tsd->weight / 10000;	// Is this how it should work when it says "the target gets add. damage based on its weight"? [LimitLine]
					//wd.div_ = 0 - wd.div_;	// If the damage adds up, let it add up.
				}
				else
					wd.div_ = -3;	// Monster target. Is this how it's supposed to work? [LimitLine]
				break;

			case SR_GATEOFHELL:
				if( sd )
					wd.div_ += (sd->spiritball_old / 2);
				break;
		}
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		{	//Calc base damage according to skill
			...
			case WM_SATURDAY_NIGHT_FEVER:
				...
				wd.damage2 = 9999;
			case SR_GATEOFHELL:
				wd.damage = wd.damage2 = (status_get_max_hp(src) - status_get_hp(src)) / 2;
				break;
			...
			switch( skill_num )
			{
				...
				case MC_CARTREVOLUTION:
					...
					if(sd && sd->cart_weight)
						skillratio += 100*sd->cart_weight/(battle_config.max_cart_weight+pc_checkskill(sd,GN_REMODELING_CART)*5000); // +1% every 1% weight
					else if (!sd)
				...
				case WS_CARTTERMINATION:
					...
					if(sd && sd->cart_weight)
						skillratio += sd->cart_weight/i * 80000/(battle_config.max_cart_weight+pc_checkskill(sd,GN_REMODELING_CART)*5000) - 100;
					else if (!sd)
				...
				case WM_METALICSOUND:
					{
						...
						if( sd )// plz check this.. is it a mistake? [pakpil]
							//skillratio += skillratio += 10 * pc_checkskill(sd, WM_LESSON);	// Need official value. [LimitLine]
							skillratio += 10 * pc_checkskill(sd, WM_LESSON);	// Need official value. [LimitLine]
						if( tsc && tsc->data[SC_DEEPSLEEP] )
				...
				case SR_KNUCKLEARROW:
					...
					{	// Knockback part of the skill. [LimitLine]
						...
						if( !map_getcell(target->m, x, y, CELL_CHKPASS) )
							skillratio *= 2;
					}
					break;
				case SR_WINDMILL:
				...
				case LG_BANISHINGPOINT:
					skillratio += 30 * skill_lv;
					if( sd )
						skillratio += 10 * pc_checkskill(sd, SM_BASH); // need official value [pakpil]
					break;
				case LG_SHIELDPRESS:
					skillratio += 200 + 20 * skill_lv; // need official value [pakpil]
					break;
				case LG_RAGEBURST:
					if( sc && sc->data[SC_FORCEOFVANGUARD] )
						skillratio += 500 + 100 * sc->data[SC_FORCEOFVANGUARD]->val3;
					break;
				case LG_EARTHDRIVE:
				...
				case LG_MOONSLASHER:
					skillratio = 50 + 50 * skill_lv;
					if( sd )
						skillratio += 10*pc_checkskill(sd, LG_OVERBRAND);
					break;
				case GN_CART_TORNADO: // FIXME: need know if the cart weight affect to the damage.
					skillratio += 100 + 50 * skill_lv + 20 * pc_checkskill(sd,GN_REMODELING_CART);
					if(sd && sd->cart_weight)
						skillratio += 100*sd->cart_weight/(battle_config.max_cart_weight+pc_checkskill(sd,GN_REMODELING_CART)*5000); // +1% every 1% weight
					else if (!sd)
						skillratio += 100; //Max damage for non players.
				...
				case GN_CARTCANNON:
					skillratio += 350 + 50 * skill_lv;
					if( sd )
						skillratio += 20 * pc_checkskill(sd, GN_REMODELING_CART) + 2 * sd->status.int_;
					break;
	...
	{
		if( tsc->data[SC_FORCEOFVANGUARD]->val3 < tsc->data[SC_FORCEOFVANGUARD]->val4 )
			tsc->data[SC_FORCEOFVANGUARD]->val3++;
	}
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Calc base damage according to skill
			...
			case NC_REPAIR:
				if( tsd )
					ad.damage = tsd->status.max_hp / 100 * (3 + 3 * skill_lv);
				break;
			case PR_ASPERSIO:
				ad.damage = 40;
				break;
			case AB_EPICLESIS:
				ad.damage = skill_lv*50; // Need official value [Rikter]
				break;
			...
			default:
			{
				...
				switch(skill_num){
					...
					case WL_COMET:
					{
						switch( skill_lv )	// This will need to be rewored in the future. [Rytech]
						{
							case 1: skillratio += 90 + rand()%200; break;
							case 2: skillratio += 110 + rand()%230; break;
							case 3: skillratio += 130 + rand()%260; break;
							case 4: skillratio += 150 + rand()%310; break;
							case 5: skillratio += 170 + rand()%320; break;
							default: break;
						}
						// Need official values. [LimitLine]
						if( distance_bl(target, src) >= 5 && distance_bl(target, src) <= 10 )
							skillratio /= 15 / 10;
						else
						if( distance_bl(target, src) >= 11 )
							skillratio /= 2;
					}
						break;
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	case MA_LANDMINE:
		...
		if (sd)
			md.damage+= 40*pc_checkskill(sd,RA_RESEARCHTRAP);
		break;
	case HT_BLASTMINE:
	...
	case RA_WUGSTRIKE:
	case RA_WUGBITE:
		if(!sd || (skill = pc_checkskill(sd,RA_TOOTHOFWUG))<=0)
				skill=0;
		// Assumed base damage formula bonus from stat. Need official. [Jobbie]
		md.damage=(sstatus->dex/10+sstatus->int_/2+skill+40)*2;
		break;
	case TF_THROWSTONE:
		md.damage=50;
		break;
	...
	case RA_ICEBOUNDTRAP:
		//official info is damage was based on skill_lv, dex, int that depends on upgrade. [Jobbie]
		md.damage = skill_lv*(100+sstatus->dex)*(100+sstatus->int_)/100;
		if (sd)
			md.damage+= 40*pc_checkskill(sd,RA_RESEARCHTRAP);
		break;
	}
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, unsigned int tick, int flag)
{
	...
	if(sc) {
		...
		{
			int rate = 10 + 2 * sc->data[SC_DUPLELIGHT]->val1; // Official rate to cast Melee and Magic Atk of Duple Light. [Jobbie]
			if( rand()%100 <= rate )
				skill_castend_damage_id(src, target, (rand()%20 < 10)?AB_DUPLELIGHT_MAGIC:AB_DUPLELIGHT_MELEE, rate, tick, SD_LEVEL);
		}

		
		if( sc->data[SC_ENERGYGAIN] && sd )
		{
			if( rand()%100 <= 10 + sc->data[SC_ENERGYGAIN]->val1 * 5 )
				pc_addspiritball(sd, skill_get_time(MO_CALLSPIRITS, sc->data[SC_ENERGYGAIN]->val1), sc->data[SC_ENERGYGAIN]->val1);
		}
	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ // Checks on actual target
		...
		case BL_SKILL:
		{
			...
			} else if (su->group->skill_id==WZ_ICEWALL || su->group->skill_id==WM_REVERBERATION || su->group->skill_id==GN_WALLOFTHORN)
			{
				state |= BCT_ENEMY;
				strip_enemy = 0;
			} else	//Excepting traps, icewall, and reverberations you should not be able to target skills.
				return 0;
		}
			break;
