...
enum {
	...
	UF_HOVER         = 0x1000,	// This unit will not trigger when a character under hovering status mov
};
...
struct skill_chain_lightning_struct
{
	int src_id, count, delay, range, targets, next;
	int bl_list[16];
};

int skill_chain_lightning(struct block_list *src, struct block_list *target, int delay, int range, int times);
int skill_chain_lightning_timer(int tid, unsigned int tick, int id, intptr data);
