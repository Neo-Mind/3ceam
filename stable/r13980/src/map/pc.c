...
{
	int tid, i;
	struct status_change *sc = status_get_sc(&sd->bl);// Added for Raising Dragon. [LimitLine]

	nullpo_retr(0, sd);

	if(sc && sc->data[SC_RAISINGDRAGON])	// Instead of adding a check for this SC everytime pc_addspiritball is called, let us just add it here. [LimitLine]
...
int pc_itemheal(struct map_session_data *sd,int itemid, int hp,int sp)
{
	...
	if(sp) {
		bonus = 100 + (sd->battle_status.int_<<1)
			...
			+ pc_checkskill(sd,AM_LEARNINGPOTION)*5
			+ pc_checkskill(sd,WM_LESSON)*5;	// Need official value. [LimitLin]
		if (potion_flag > 1)
			bonus += bonus*(potion_flag-1)*50/100;
		if(bonus != 100)
			sp = sp * bonus / 100;
...
int pc_setriding(TBL_PC* sd, int flag)
{
	...
	if( flag )
	{		
		if( pc_checkskill(sd,skillnum) > 0 ) // Check if you have the necessary skill to mount.ù
