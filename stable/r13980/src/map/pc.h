...
struct map_session_data {
	...
	struct {
		...
		unsigned no_gemstone : 2; // Increased to 2 to support Adoramus effect
	...
	int chain_counter;	// Chain Lightning Counter [LimitLine]
	int chain_id;		// Last target hit by Chain Lightining.
...
#define pc_ishiding(sd)       ( (sd)->sc.option&(OPTION_HIDE|OPTION_CLOAK|OPTION_CHASEWALK) || (sd)->sc.data[SC_CAMOUFLAGE] )
#define pc_iscloaking(sd)     ( !((sd)->sc.option&OPTION_CHASEWALK) && ((sd)->sc.option&OPTION_CLOAK) )
#define pc_iscamouflage(sd)   ( !((sd)->sc.data[SC_CAMOUFLAGE]) )
#define pc_ischasewalk(sd)    ( (sd)->sc.option&OPTION_CHASEWALK )
#define pc_iscarton(sd)       ( (sd)->sc.option&OPTION_CART )
#define pc_isfalcon(sd)       ( (sd)->sc.option&OPTION_FALCON )
#define pc_isriding(sd)       ( (sd)->sc.option&OPTION_RIDING && !((sd)->class_&JOBL_THIRD) )
