...
int atcommand_param(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if( pc_isGM(sd) < battle_config.gm_lvl_can_override_maxparam )
...
int atcommand_mapinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	case 2:
		...
		for (i = 0; i < map[m_id].npc_num;)
		{
			...
			sprintf(atcmd_output, "NPC %d: (ID: %d) %s | Direction: %s | Sprite: %d | Location: %d %d",
				++i, nd->bl.id, nd->name, direction, nd->class_, nd->bl.x, nd->bl.y);
			clif_displaymessage(fd, atcmd_output);
		}
		break;
	case 3:
 ...
 *
 *------------------------------------------*/
int atcommand_get_wug(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	nullpo_retr(-1, sd);

	if(pc_isriding(sd, OPTION_RIDING_WUG)) // player is riding a warg.
	{
		clif_displaymessage(fd, msg_txt(708));
		return -1;
	}
	...
	if(!pc_iswarg(sd)) { // if actually no warg
		if(!pc_checkskill(sd, RA_WUGMASTERY))
		{
			clif_displaymessage(fd, msg_txt(705)); // This player cannot have a Warg with his/her current job.
			return -1;
		}

		if (sd->disguise)
		{
			clif_displaymessage(fd, msg_txt(707)); // You cannot mount a Warg while in disguise.
			return -1;
		}

		pc_setoption(sd, sd->sc.option | OPTION_WUG);
		clif_displaymessage(fd, msg_txt(704)); // You have obtained a Warg.
	} else {	//Dismount
		pc_setoption(sd, sd->sc.option & ~OPTION_WUG);
		clif_displaymessage(fd, msg_txt(706)); // This player's Warg has been released.
	}
...
AtCommandInfo atcommand_info[] = {
	...
	{ "getwarg",           20,20,     atcommand_get_wug },
