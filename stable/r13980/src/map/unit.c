...
int unit_run(struct block_list *bl)
{
	...
	TBL_PC * sd = map_id2sd(sc->data[SC_RUN]->val3);
...
int unit_resume_running(int tid, unsigned int tick, int id, intptr data)
{
	...
	if(sd && pc_isridingwarg(sd))
		clif_skill_nodamage(ud->bl,ud->bl,RA_WUGDASH,ud->skilllv,
			sc_start4(ud->bl,status_skill2sc(RA_WUGDASH),100,ud->skilllv,unit_getdir(ud->bl),0,0,0));
	else
...
int unit_skillcastcancel(struct block_list *bl,int type)
{
	...
	if( sc && sc->data[SC_HUMMINGVOICE] )
		return 0;	// Character under Unlimited Humming Voice have their skills un-cancellable. [LimitLine]
